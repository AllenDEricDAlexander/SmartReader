...
- 支持全文搜索、页码跳转、缩放、适合宽度、适合页面；搜索会给出真实的匹配总数、当前匹配序号，以及可点击跳转的匹配结果列表（含页码和上下文摘要）。
- 支持 per-tab 阅读历史，明确跳转才进入后退/前进历史。
- 支持书签新增、列表、跳转、删除和 SQLite 持久化。
- 选中 PDF 文本后可在浮层中选择颜色，并标注为高亮、下划线，或生成锚定原文的笔记；页面级笔记仍可从工具栏创建。批注由 SmartReader 管理，不写回原始 PDF。
- 支持批注列表、跳转、删除、可编辑笔记、标签、JSON 导入和 JSON 导出。
- 支持文档收藏，以及批注和笔记的标签创建、重命名、合并、删除和关联计数。
...
- 标签由 SmartReader 管理，当前界面可关联批注和笔记；文档标签关系保留在持久层中。
